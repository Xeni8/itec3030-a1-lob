...
public class Exchange {
	...
	public void submitOrder(IOrder o, long time) {
		...
		if (o instanceof Bid) {// Order is a bid
			...
			if (Outcome.getUnfulfilledOrder().getQuantity() > 0) {
		...
		for (ITrade t:oOutcome.getResultingTrades()) {
			...
			accounts.getTraderAccount(t.getBuyer()).applyTradePayment(t.getTotalValue());
			...
			accounts.getTraderAccount(t.getSeller()).receiveTradePayment(t.getTotalValue());
